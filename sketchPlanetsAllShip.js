//Vou colocar algumas estrelas: w piscium, delta aquarii, beta cetti
...
function setup() {
  ...
  zcamSlider =createSlider(500,2000);//***
...
//constelações
   push();
   translate(0,0,0);
   rotateZ(PI/2.3);  //ajusta a data - 2.3 é o valor certo.  Não mude
   //rotateX(PI/2);
   rotateX(-PI/3);  //melhor valor é -pi/3
   //rotateY(-PI/6);
   //rotateZ(PI/4);
   //texture(sky);
   fill(0);
  ...
  pop();
   ...
   posycam += deltay;
   rotabs = abs(rotatexx);
   camera.setPosition(1.7*posxe, 1.7*posye, mouseX);  // na eclítica além da Terra,
   //camera.setPosition(mouseX, mouseY, mouseY);  // de cima do Sol,
   camera.lookAt(0,mouseY,mouseX);  // para o Sol,
   ...
   //pop();
    ...
    push();
         textFont(myfont, 40 );
      //rotateZ(PI/2);
      //text(diaEHora1 + '\n' + diaEHora,1.2*posxe,1.2*posye);
      
      //text(diaEHora1 + '\n' + diaEHora,0,0.2*posxe);
    pop();
    //Draws Mars, Earth
    push();
    ...
    //rotateX(-PI/4);
     textFont(myfont, 40 );
  text('Credito do mapa estelar: NASA/Goddard Space Flight Center Scientific Visualization Studio.',50,0,0);
  }
...
function mouseWheel(event) {
  //controla component x da camera
  rotatexx += event.delta;
  console.log(rotatexx);
