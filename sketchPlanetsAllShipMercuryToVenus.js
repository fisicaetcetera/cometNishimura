...
function setup() {
  ...
  d0 = numeroDeDias();
  diasj20000 = diasj2000;
  //diasj2000 = epocaMoon / 86400;  //Declarado no main
  //console.log('diasj2000, numdias = ' + diasj2000 +' ' + d0);
  d = d0;
  Mars();
  posxe = Xe*150 ;
  posye = Ye*150;
...
function numeroDeDias() {
  ...
  let epocaj2000 = (timestamp - stampj2000) / 1000;
  let epoca = (timestamp - stampj2013) / 1000;
  //console.log('epoca = ' + epoca);
  let numdias = epoca / 86400;
  diasj2000 = epocaj2000 / 86400;  //Declarado no main
...
function Moon() {
  ...
  Nm = FNrange((125.1228 - 0.0529538083 * diasj2000) * rads); //Long ascending node
  im = 5.1454 * rads; // inclination of orbit 
  wm = FNrange((318.0634 + 0.1643573223 * diasj2000) * rads); //argm of periapsis
  ws = FNrange((282.9404 + 4.70935e-5 * diasj2000) * rads);
  am = 60.2666 //semi-major axis (Earth radii)
  as = 1; //(AU)
  ecm = 0.0549 * rads;
  ecs = (0.016709 - 1.151E-9 * diasj2000) * rads; //
  Mm = FNrange((115.3654 + 13.0649929509 * diasj2000) * rads); // mean anomaly
  Ms = FNrange((356.0470 + 0.9856002585 * diasj2000) * rads);
  Em = Mm + ecm * sin(Mm) * (1 + ecm * cos(Mm)); //eccentric anomaly
  //console.log('Em = ', Em, Em * degs)
  Es = Ms + ecs * sin(Ms) * (1 + ecs * cos(Ms));
  // me = ne * diasj2000 + Le - we; //earth
