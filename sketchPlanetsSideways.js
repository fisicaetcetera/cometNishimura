...
let ii; // number of days before or after today
...
function setup() {
  ...
  //ii = -int(377.5*365); //year of 1645: Maunder min
  //ii = -int(2434.5*365); //23 August -413 (-412)

  //ii = -int(2027.6*365); //ano -5, 6 AC
  //ii = -int(14.2*365);
  //ii = -465; // Número de dias antes
  ...
  zcamSlider =createSlider(100,2500,2500);
  ...
  text('recent',-width/2+20,-height/2+20,0);
   ...
   ii += 0.04;
   if (ii < ifim+5) { //till 30 days after
  ...
  pop();
    ...
    pointLight(111,0,0,posxe,posye,0);
    
        zcam = 600 - 300*exp(-frameCount/40);
        zcam = zcamSlider.value();
        //zcam = 500;
    //camera.setPosition(posxe/1.8,posye/1.8,0);
    //camera.lookAt(posxe,posye,0);
    camera.setPosition(posxe+1.5*posxmoon,posye+1.5*posymoon, 222);
    camera.lookAt(posxe,posye,0);
    //
    //sol
    push();
...
function jupiter() {
  ...
  distance = sqrt(Xj * Xj + Yj * Yj + Zj * Zj); //rel to sun
  ...
  //jupdat.print([RA_hours + " " + Dec_degrees + " " + distance]);
  //  
...
function Saturn() {
  ...
  //satdat.print([RA_hours + " " + Dec_degrees + " " + distance]);
  //  
} //end function Saturn()
//
//
...
function Moon() {
  ...
  segundos = now.getSeconds();
      ...
      text(diaEHora1, -150, -200); //day-month-year
      text(diaEHora, -150, -220); //hour-minutes-seconds
      pop()
