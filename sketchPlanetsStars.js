...
//incluindo estrelas
//muito bom, com quatro indicações de constelações
//Essa cópia é para usar com eventos com resolução
//diária, i.e. iteração de 1 dia.
//colocando camera
//will include zoom to show the rover and Mars at close.
//a trajetória está em valores absolutos - a do rover.
//preciso mudar isso - antes da aterrisagem
//  The authors I follow I Started with
//  Moon positions to a few minutes of arc
//   From Paul Schlyter's page at
//   (more recently here: http://astro.if.ufrgs.br/trigesf/position.html.)
//   As these authors say:
//   "The QBASIC code here 
//   (https://paulsite.com//calculation_of_right_ascension_and_declination/)
//   is a literal translation of Paul's
//   formulas.
// TIME
...
function setup() {
  ...
  ec_rads = ec * rads;
  //
  cnv = createCanvas(1366, 768, WEBGL);
    camera = createCamera();
    setCamera(camera);
  background(0)
  ...
  posxe = Xe*150 ;
  posye = Ye*150;
        rpc0 = sqrt(posxe*posxe + posye*posye);
      angleInicial = atan2(posye,posxe);
      console.log('angulo inicial, rpc0 = ', angleInicial,rpc0,rpc);
  d0rb1 = drb1;
  ano = now.getFullYear();
  mes = now.getMonth() + 1;
  dia = now.getDate();
  ...
  //
  ii = -35 // dias anteriores 
  ifim = 35 // dias posteriores.
  //
  ...
  //desenha estrelas
    ...
    translate(0, 0, 0)
    //rotateY(angle);
    starx = star[i].galX * 70;
    stary = -star[i].galY * 70 ;
    starz = star[i].galZ * 70 ;
    cor = star[i].color;
    ...
    fill(cor);
      ...
      if(i<30){
        textFont(myfont, 15);
        text(star[i].starName,10,23,23);
      }
    sphere(1);
    pop();
  }
  
  // fim do desenho das estrelas
   ...
   if (ii++ < ifim) { //
    ...
    posymer = oy - 150 * Ymer; //y coordinate of mercury  
    poszmer = oz + 150 * Zmer; //z coordinate of mercury   
    posxe = ox + 150 * Xe; // same, for earth
    posye = oy - 150 * Ye;
    stereoADist = sqrt(posxe*posxe + posye * posye);
  ...
  fill(111);
  textFont(myfont, 35);
  //text(diaEHora1 + '\n' + diaEHora,posxms+20, posyms-20,0);
  text(diaEHora1 + '\n' + diaEHora,-50, height/4, 0);
    //
   // zcam = 1000;
    //zcam = 700 + 2500*exp(-frameCount/45);
    zcam = 700 + (400 + 330*sin(frameCount/22));
    stereoX = stereoADist * cos(stereoALong);
    stereoY = -stereoADist * sin(stereoALong);
    //camera.setPosition(stereoX+10, stereoY+10,0);
    angulocam = frameCount/50;
    distanciaterra = sqrt(posxe*posxe+posye*posye);
    camera.setPosition(distanciaterra*sin(angulocam),distanciaterra*cos(angulocam),zcam);
    //camera.setPosition(3*posxe, 3*posye,0);
    //camera.lookAt(posxsat/3,posysat/3,0);
    //camera.lookAt(posxj,posyj,0);
    camera.lookAt(0,0,0);
    //camera.pan(sin(frameCount/48));
    ...
    push();
      ...
      stroke('blue');
      strokeWeight(1);
      line(posxe, posye, 0, posxv, posyv, poszv);
    //
    pop();
    //Draws stereo A
    push(); 
    ...
    //strokeWeight(1);
      textFont(myfont, 15);
      text('SA',stereoX-10, stereoY-10, 0);
      //console.log('Stereo A: ', stereoADist, stereoX, stereoY, stereoALong);

    pop();
      stroke('grey');
      
      strokeWeight(1);
    line(stereoX, stereoY, 0, -25*stereoX, -25*stereoY,0);
    //
    // Draws Tesla
    //
    push();
...
function Mars() {
  ...
  stereoALong = earthLong - 0.9075; //behind by ~52 degrees
  //console.log('stereoLong : ', stereoALong, earthLong);
  //
  //////console.log(' re, ve, pe = ', re, ve, pe);
  // ////console.log('X, Y, Z = ', X, Y, Z);
  //////console.log('Xe, Ye, Ze = ', Xe, Ye, Ze);
