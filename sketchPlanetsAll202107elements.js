...
//let Le = 301; //same as L, for earth
...
let Xv, Yv, Zv;
let Xmer, Ymer, Zmer;
let Xgo20, Ygo20, Zgo20; //asteroid
let camera;
...
let poszgo20;
let posxms, posyms;
let posxcam, posycam, poszcam; //camera
...
function setup() {
  ...
  ec_rads = ec * rads;
  //
  knvs = createCanvas(750, 620, WEBGL);
  knvs.mouseWheel(changeSize);
  camera = createCamera();
  setCamera(camera);
  ...
  //text('recent',-width/2+20,-height/2+20,0);
   ...
   if (ii++ < ifim) { //till 30 days after
    ...
    diasgo20 = diasgo200 + ii;
    console.log('diasgo20 = ', diasgo20);
    //calcula data
  ...
  //pop();
    ...
    //////console.log('ii , day = ', ii, d);
      ...
      raioGo20 = raioMercury/4; //change this
      let tint = 0.;
    let tout = 0.;
    noStroke();
    //
    ...
    posxgo20 = ox + factor * Xgo20;//A.U.
    posygo20 = oy - factor * Ygo20;
    poszgo20 = oz + factor * Zgo20;
  ...
  case 'mercury':
      iden +=1;
      deltax = (posxmer - posxcam)/(5);
      deltay = (posymer - posycam)/(5);
      //console.log('posxmer, posxcam = ', posxmer, posxcam);
      ...
      if(frameCount > 199){
        planet = 'venus';
        iden =0;
        console.log('saindo de mercurio======');
        //console.log(posymer,posycam);
      }
    break;
   case 'venus':
      console.log('to venus');

      iden += 1;
      deltax = (posxv - posxcam)/(10);
      deltay = (posyv - posycam)/(10); 
      //console.log(posyv,posycam,deltay);
      camera.setPosition(0,0,zcam);
      camera.lookAt(0,0,0);
      if(frameCount > 399){
      //camera.setPosition(posxcam,posycam,zcam);
      //camera.lookAt(0,0,0);
      iden = 0;
        planet = 'earth';
      }
      break;
         case 'earth':
      console.log('to earth');

      iden += 1;
      deltax = (posxe - posxcam)/(10);
      deltay = (posye - posycam)/(10); 
      //console.log(posye,posycam,deltay,iden);
      camera.setPosition(0,0,zcam);
      camera.lookAt(0,0,0);
      if(frameCount > 599){
            camera.setPosition(posxcam,posycam,zcam);
      ...
      iden += 1;
      deltax = (posxms - posxcam)/(10);
      deltay = (posyms - posycam)/(10); 
      //console.log(posyms,posycam,deltay,iden);
      camera.setPosition(0,0,zcam);
      camera.lookAt(0,0,0);
      if(frameCount > 799){
        planet = 'jupiter';
      ...
      break;
      case 'jupiter':
      console.log('to jupiter');
      iden += 1;
      if(!close){
      deltax = (posxj - posxcam)/(10);
      deltay = (posyj - posycam)/(10);
      } else {
        deltax = 0;
        deltay = 0;
      } 
      distancia = 10*sqrt(deltax*deltax + deltay*deltay);
      if(distancia < 2*raioJupiter){
        close = true;
      }
      camera.setPosition(posxcam,posycam,zcam);
      ...
      break;
      case 'saturn':
      console.log('to saturn');

      iden += 1;
      deltax = (posxSat - posxcam)/(10);
      deltay = (posySat - posycam)/(10); 
      distancia = 10*sqrt(deltax*deltax + deltay*deltay);
      if(distancia < 2*raioSaturn){
      deltax=0;
      deltay=0;
      deltaz = 0;
      }
      camera.setPosition(posxcam,posycam,zcam);
      camera.lookAt(posxSat,posySat,0);
      //console.log(posyms,posycam,deltay,iden);
      ...
      break;
      case 'uranus':
      console.log('to uranus');

      iden += 1;
      deltax = (posxUra - posxcam)/(10);
      deltay = (posyUra - posycam)/(10); 
      distancia = 10*sqrt(deltax*deltax + deltay*deltay);
      if(distancia < 2*raioUranus){
      deltax = 0;
      deltay = 0;
      
      }      
      camera.setPosition(posxcam,posycam,poszUra);
      camera.lookAt(posxUra,posyUra,poszUra);
      //console.log(posyms,posycam,deltay,iden);
      ...
      break;
      case 'neptune':
      console.log('to neptune');

      iden += 1;
      deltax = (posxNep - posxcam)/(10);
      deltay = (posyNep - posycam)/(10); 
      distancia = 10*sqrt(deltax*deltax + deltay*deltay);
      if(distancia < 2*raioNeptune){
      deltax=0;
      deltay=0;
      }      
      camera.setPosition(posxcam,posycam,zcam);
      camera.lookAt(posxNep,posyNep,poszNep);
      //console.log(posyms,posycam,deltay,iden);
      ...
      break;
      case 'pluto':
      console.log('to pluto');

      iden += 1;
      deltax = (posxPlu - posxcam)/(10);
      deltay = (posyPlu - posycam)/(10); 
      deltaz = (poszPlu - poszcam)/(10);  
      distancia = 10*sqrt(deltax*deltax + deltay*deltay + deltaz*deltaz);
      if(distancia < 11*raioPluto){
      deltax = 0;
      deltay = 0;
      }
      camera.setPosition(posxcam,posycam,poszPlu);
      camera.lookAt(posxPlu,posyPlu,poszPlu);
      //console.log(posyms,posycam,deltay,iden);
      ...
      break;
      default:
      //colocar aqui as definições iniciais
      //posxcam = ...
      //nomeia o primeiro planeta aqui
      deltax = 0;
      deltay = 0;
      } //do switch 
   ...
   //camera.setPosition(0, 0,zcam);
   //camera.lookAt(0,0,0);
   //console.log('poscam, posvenus: ', posxcam,posycam,posxv,posyv);
   //plots ship motion
   push();
   emissiveMaterial(0, 0, 255);
   fill('blue')
   translate(posxcam, posycam, 0);
   sphere(2);
   pop();
    ...
    //===== lines to s and j always - commented out at present
      ...
      strokeWeight(1);
      stroke('yelow');
      //line(posxms, posyms, posxSat, posySat);avicon
      stroke('green');
      noFill();
      strokeWeight(1);
      line(posxv, posyv, posxJ, posyJ);
      noStroke();
    ...
    //Draws Mars, Earth, etc
    //
    // 2008 GO20
    push();
    translate(posxgo20, posygo20, poszgo20);
    fill(100,111,111);
    box(10);
...
function changeSize(event) {
  ...
  //console.log('inside changeSize, zcam =', zcam);
...
function numeroDeDias() {
  ...
  stampj2000 = (new Date('1999-12-31T00:00:00Z')).getTime();
  stampj2021 = (new Date('2021-07-01T00:00:00Z')).getTime();
  //
  //////console.log('stampj2013 = ', stampj2013);
  // epoca, em milisegundos
  let epocaMoon = (timestamp - stampj2000) / 1000;
  let epocago20 = (timestamp - stampj2021) /1000;
  let epoca = (timestamp - stampj2013) / 1000;
  //console.log('epoca = ' + epoca);
  let numdias = epoca / 86400;
  diasMoon = epocaMoon / 86400;  //Declarado no main
  diasgo20 = epocago20/86400;
...
function go20() {
  //
  // the 'j' is not important
  //

  ij = 2.249858058757811;//inclination
  oj = 119.5822524225735;//longitude of ascending node j2000
  pj = 160.65552070913;//argument of perihelion
  aj = 1.893204117483213;//Semimajor axis
  nj = 0.378362;//daily motion
  ej = 0.491108; //Eccentricity
  mj = 308 //mean anomaly, 20210725
  //Lj = mj + wj // mean longitude, 20210725

  me_degrees = ne * diasgo20 + me; //earth
  mj_degrees = nj * diasgo20 + mj; //asteroid
  ...
  //True anomaly for asteroid  
  vj = mj + (2 * ej - 0.25 * pow(ej, 3) + 5 / 96 * pow(ej, 5)) * sin(mj) +
    (1.25 * pow(ej, 2) - 11 / 24 * pow(ej, 4)) * sin(2 * mj) +
    (13 / 12 * pow(ej, 3) - 43 / 64 * pow(ej, 5)) * sin(3 * mj) +
    103 / 96 * pow(ej, 4) * sin(4 * mj) + 1097 / 960 * pow(ej, 5) * sin(5 * mj);
    //vj = mj;  // dá o mesmo resultado
  //
  //console.log('v = ', vj);
  //
  //True anomaly for earth  
  ...
  // Calculate  coordinates X, Y, and Z, for planet
  //
  Xgo20 = rj * [cos(oj) * cos(vj + pj - oj) - sin(oj) * sin(vj + pj - oj) *
    cos(ij)
  ]
  Ygo20 = rj * [sin(oj) * cos(vj + pj - oj) + cos(oj) * sin(vj + pj - oj) *
    cos(ij)
  ]
  Zgo20 = rj * [sin(vj + pj - oj) * sin(ij)]
  //********* From stars point o view, till here **********
  // The quantity v + p - o is the angle of the planet       // measured in the plane of the orbit from the ascending node
  //////console.log('X, Y, Z = ', X, Y, Z);
  // Verify that this is correct: in case you used angles
  ...
  Dec_degrees = Dec_radians * degs;
  //
  ////console.log('go20: ', RA_hours, Dec_degrees, distance);
  //  
} //end function go20()
//
...
function rb12016() {      // 2016RB1
  ...
  let oe = 180.0; //same as o, for earth
...
function Moon() {
  ...
  segundos = now.getSeconds();
      ...
      //text(diaEHora1, -150*500/(zcam+1), -250*500/(zcam+1); //day-month-year
      text(diaEHora1, -150*500/zcam, -250*500/zcam); //day-month-year
