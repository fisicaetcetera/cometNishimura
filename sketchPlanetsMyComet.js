...
let rpc;
  ...
  let xcomN =0, ycomN=0, zcomN=0; //comet Nishimura
  let deltateta = 0.056;
...
//constelações
  ...
  pop();
    ...
    Mercury(); //Xmer, etc.
    comet21f1(); //xcom, ycom
    cometNishimura2023(); // xcomN, ycomN
   ...
   //camera(distancia*sin(omega)*sin(tetaz), distancia*cos(omega)*sin(tetaz),distancia*cos(tetaz),posx,posy,posz,0,1,0);
    ...
    //rotateX(PI/2);
     textFont(myfont, 10 );
  text('Programado em p5.js/WEBGL por Enivaldo Bonelli',0,0,0);
  ii = - 10
  }
...
function Moon() {
  ...
  segundos = now.getSeconds();
      ...
      //if(distancia<1500){
      fill(255,0,0,80);
      text(diaEHora1, -50, -30); //day-month-year
      text(diaEHora, -50, -50); //hour-minutes-seconds
      //}
      pop()
...
function comet21f1() {
  ...
  //the end
  //
  // COMETA NISHMURA 09/2023
//
//
function cometNishimura2023() {
  //Used expressions for near parabolic orbits
  //from Paul Schlyter's work
  k = 0.01720209895;
  e = 0.9934651;  //
  q = 0.2247741; //au - distance at perihelion
  qqq = 0.011356;
  i = 2.31;  //rad, inclination
  w = 2.03; //rad, arg perihelion
  N= 1.1655; // Longitude of the ascending node
  tp = 8660.65; //date of perhelion
  //d-tp = time till the perihelion
  a = 0.75*(diasMoon-tp) * k * sqrt((1+e)/qqq);
  console.log("diasMoon =  ", diasMoon);
  b = sqrt(1+a*a);
  W = Math.cbrt(b+a) - Math.cbrt(b-a);
  WW = W*W;
  WWWW = WW*WW;
  ...
  console.log(d, tp);
  console.log(xcomN, ycomN,zcomN,rN,v);
  } //end of function cometNishimura2023
  //the end
  //
